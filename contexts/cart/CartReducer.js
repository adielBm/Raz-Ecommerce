...
export const CartReducer = (previousState, { type, payload }) => {
  ...
  switch (type) {
    case 'ADD_ITEM':
      if (
        !state.items ||
        !state.items.find((item) => item.slug === payload.product.slug)
      ) {
        state.items.push({
          ...payload.product,
        ...
        state.items[
          state.items.findIndex((item) => item.slug === payload.product.slug)
        ].quantity += payload.count
      }
      break
    ...
    case 'INCREASE':
      state.items[state.items.findIndex((item) => item.slug === payload.slug)]
        .quantity++
      break
